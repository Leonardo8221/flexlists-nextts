...
async function updateTranslationKey(id: number, name: string, type: TranslationKeyType, ownerId: number, reusable: boolean, config: string): Promise<FlexlistsError | FlexlistsSuccess<TranslationKeyDto>> {
  var response = await axios.post<FlexlistsError | FlexlistsSuccess<TranslationKeyDto>>(`/api/translationKey/updateTranslationKey`, { id, name, type, ownerId, reusable, config })
...
async function getAllTranslationKey(): Promise<FlexlistsError | FlexlistsSuccess<TranslationKeyDto[]>> {
  var response = await axios.get<FlexlistsError | FlexlistsSuccess<TranslationKeyDto[]>>('/api/translationKey/getAllTranslationKey?query=' + JSON.stringify({ where: { reusable: true } }))
  return response.data;
};
