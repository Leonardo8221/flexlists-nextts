    ...
    <Box sx={{ height: '100%', display: 'grid', gridTemplateColumns: `${isDesktop ? '64px' : '24px'} repeat(7, 1fr)`, overflowY: 'auto' }}>
      ...
      {days.map((day: any, index: number) => (
        <Box key={`${index}-week`}>
          ...
          {hours.map((hour: string) => (
            <Box key={`${hour}-right`} sx={{ height: {xs: '88px', md: '98px'}, border: '1px solid rgba(0, 0, 0, 0.1)', px: {xs: 0.3, md: 1}, py: 0.5, cursor: 'pointer', overflow: 'hidden' }} onClick={(e: any) => { if(!e.target.classList.contains('edit_row')) handleData({date: format(day, 'MM/dd/yyyy') + ' ' + hour + ':00'}, day) }}>
              {getData(new Date(format(day, 'MM/dd/yyyy') + ' ' + hour + ':00'), 'hour').map((data: any) => (
                <Box key={`${data.id}-week`} className="edit_row" sx={{ width: '100%', whiteSpace: 'nowrap', textOverflow: 'ellipsis', display: 'flex', cursor: 'pointer', '&:hover': { color: theme.palette.palette_style.text.selected }, borderRadius: '20px', backgroundColor: '#FFB7B7', marginBottom: {xs: '2px', md: '5px'}, fontSize: '12px' }} onClick={() => handleData(data, day)}>
                  <Box sx={{ display: 'flex', width: '100%', px: 1, py: 0.2, borderRadius: 1.5, textTransform: 'capitalize', backgroundColor: theme.palette.palette_style.background.selected }} className="edit_row">
                    ...
                    <Box className="edit_row">{getDataStatus(data, new Date(format(day, 'MM/dd/yyyy') + ' ' + hour + ':00'), 'hour') === 'begin' ? getFieldData(data, 'begin').split(' ')[1] : hour}</Box>
