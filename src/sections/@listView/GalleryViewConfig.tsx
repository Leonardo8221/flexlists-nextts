...
function GalleryViewConfig({ submit, updateConfig, columns, availableFieldUiTypes }:GalleryViewConfigProps)
{
  ...
  const [isOpenAvatarFieldModal, setIsOpenAvatarFieldModal] = useState<boolean>(false);
  const [isOpenNameFieldModal, setIsOpenNameFieldModal] = useState<boolean>(false);
  const [isOpenImportanceFieldModal, setIsOpenImportanceFieldModal] = useState<boolean>(false);
  const [isOpenDescriptionFieldModal, setIsOpenDescriptionFieldModal] = useState<boolean>(false);
  const avatarFieldUiTypes : FieldUIType[] = availableFieldUiTypes.filter((uiType) => uiType.name === FieldUiTypeEnum.Image);
  const nameFieldUiTypes : FieldUIType[] = availableFieldUiTypes.filter((uiType) => uiType.name === FieldUiTypeEnum.Text);
  const importanceFieldUiTypes : FieldUIType[] = availableFieldUiTypes.filter((uiType) => uiType.name === FieldUiTypeEnum.Choice);
  const descriptionFieldUiTypes : FieldUIType[] = availableFieldUiTypes.filter((uiType) => uiType.name === FieldUiTypeEnum.LongText);
  ...
  const [avatarFields, setAvatarFields] = useState<ViewField[]>(getAvatarFields());
  const [nameFields, setNameFields] = useState<ViewField[]>(getNameFields());
  const [importanceFields, setImportanceFields] = useState<ViewField[]>(getImportanceFields());
  const [descriptionFields, setDescriptionFields] = useState<ViewField[]>(getDescriptionFields());
  ...
  const reloadColumns = () =>
  {
    ...
    updateGalleryConfig(newAvatarFields.length > 0 ? newAvatarFields[0].id : 0, newNameFields.length > 0 ? newNameFields[0].id : 0, newImportanceFields.length > 0 ? newImportanceFields[0].id : 0, newDescriptionFields.length > 0 ? newDescriptionFields[0].id : 0);
  }
  ...
  const onAvatarFieldChange = (event: SelectChangeEvent) => {
    ...
    updateGalleryConfig(convertToInteger(value), nameFieldId, importanceFieldId, descriptionFieldId);
  };
  ...
  const onNameFieldChange = (event: SelectChangeEvent) => {
    ...
    updateGalleryConfig(avatarFieldId, convertToInteger(value), importanceFieldId, descriptionFieldId);
  };
  ...
  const onImportanceFieldChange = (event: SelectChangeEvent) => {
    ...
    updateGalleryConfig(avatarFieldId, nameFieldId, convertToInteger(value), descriptionFieldId);
  };
  ...
  const onDescriptionFieldChange = (event: SelectChangeEvent) => {
    ...
    updateGalleryConfig(avatarFieldId, nameFieldId, importanceFieldId, convertToInteger(value));
  };

  const updateGalleryConfig = (newDateFieldId: number, newNameId: number, newImportanceId: number, newDescriptionId: number) =>
  {
    updateConfig({avatarId: newDateFieldId, nameId: newNameId, importanceId: newImportanceId, descriptionId: newDescriptionId});
  };
  
  return (
    <Box sx={{ pt: 2 }}>
      <Box sx={{ pt: 2 }}>
        <Typography variant="subtitle2" gutterBottom>Avatar</Typography>
        <Select
          value={`${avatarFieldId}`}
      ...
      <Box sx={{ pt: 2 }}>
        <Typography variant="subtitle2" gutterBottom>Task Name</Typography>
        <Select
          value={`${nameFieldId}`}
          required
        ...
        >
          {nameFields.map((nameColumn: ViewField) => (
            <MenuItem key={`${nameColumn.id}`} value={`${nameColumn.id}`} >{nameColumn.name}</MenuItem>
          ))}
          <MenuItem key={"-1"} value={"-1"} >create a new field</MenuItem>
        </Select>
      </Box>
      <Box sx={{ pt: 2 }}>
        <Typography variant="subtitle2" gutterBottom>Importance</Typography>
        <Select
        ...
        >
          {importanceFields.map((importanceColumn: ViewField) => (
            <MenuItem key={`${importanceColumn.id}`} value={`${importanceColumn.id}`} >{importanceColumn.name}</MenuItem>
          ))}
          <MenuItem key={"-1"} value={"-1"} >create a new field</MenuItem>
        </Select>
      </Box>
      <Box sx={{ pt: 2 }}>
        <Typography variant="subtitle2" gutterBottom>Task Description</Typography>
        <Select
        ...
        >
          {descriptionFields.map((descriptionColumn: ViewField) => (
            <MenuItem key={`${descriptionColumn.id}`} value={`${descriptionColumn.id}`} >{descriptionColumn.name}</MenuItem>
          ))}
          <MenuItem key={"-1"} value={"-1"} >create a new field</MenuItem>
        </Select>
      </Box>
